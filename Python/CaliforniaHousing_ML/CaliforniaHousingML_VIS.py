...
#Step 21 - Adding visual for Actual vs Predicted to model fit quality so I can see how accurate the model is overall.
models = [LR, HGBR, RFR]
...
#Step 22 - Adding visual for Residual plot for error structure, this is to see if the model is biased or missing patterns.
for model, name in zip(models, model_names):
    y_pred = model.predict(x_test)
...
#Step 23 - Adding visual Feature importance for model interpretation to see which variables matter the most for prediction.
